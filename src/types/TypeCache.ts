...
import { TypeCode } from "./TypeSystem";
import { TypeDetector, globalTypeDetector } from "./TypeDetector";
...
export class TypeCache {
  ...
  getType(value: unknown): TypeCode {
    ...
    if (typeof value !== "object") {
      if (this.trackStats) this.stats.primitiveChecks++;
    ...
    if (this.trackStats) this.stats.objectLookups++;

    const cached = this.objectCache.get(value);
    if (cached !== undefined) {
      if (this.trackStats) this.stats.cacheHits++;
      return cached;
    }

    // Cache miss: detect and store
    if (this.trackStats) this.stats.cacheMisses++;
...
export const cachedValidateType = (
  value: unknown,
  code: TypeCode
): boolean => globalTypeCache.validateType(value, code);
